...
=======
  let copyName = document.querySelector(".itemInputPrime");
  let copyDesc = document.querySelector(".itemDescPrime");
  let destName = document.querySelector(
    "ul.saved-items li:last-child .itemInput"
  );
  let destDesc = document.querySelector(
    "ul.saved-items li:last-child .itemDesc"
  );
  destName.value = copyName.value;
  destDesc.value = copyDesc.value;
}
...
function disableCheck() {
  for (
    let i = 0;
    i < document.querySelectorAll(".saved-items li").length;
    i += 1
  ) {
    if (i == 0) {
      document.getElementsByClassName("btnUp")[0].setAttribute("disabled", "");
      ...
      document
        .getElementsByClassName("btnDown")
...
=======
  for (
    ...
    if (
      i == 1 &&
      document.getElementsByClassName("btnUp")[1].disabled == true
    ) {
      document.getElementsByClassName("btnUp")[1].disabled = false;
    } else if (
      i == document.querySelectorAll(".saved-items li").length - 2 &&
      document.getElementsByClassName("btnDown")[i].disabled == true
    ) {
      document.getElementsByClassName("btnDown")[i].disabled = false;
    }
  }
  ...
  for (i = 0; i < document.querySelectorAll("[type=checkbox]").length; i += 1) {
    if (document.querySelectorAll("[type=checkbox]")[i].checked) {
      document
        ...
        [i].nextElementSibling.firstChild.classList.add("strike");
    } else {
      //removes strike-through
      document
        .querySelectorAll("[type=checkbox]")
        [i].nextElementSibling.firstChild.classList.remove("strike");
    }
  }
});
...
document.addEventListener("keydown", e => {
  if (
    ...
    document.activeElement ==
      document.getElementsByClassName("itemDescPrime")[0]
  ) {
    document.getElementsByClassName("save")[0].click();
  }
});
...
//
// MULTICLICK USING SHIFT
//

///SECOND is to then get the clickHandler function working to handle the actual checkmarking process - check for checked state, shift key, etc
...
document.addEventListener("click", function(event) {
  if (
    ...
    event.target == document.getElementById("cal-modal")
  ) {
    document.getElementById("cal-modal").style.display = "none";
  }
});
